...
test("configured edge map remains available for local and explicit overrides", () => {
  assert.equal(
    selectedConfiguredEdgePlaybackBaseUrl(headers({ "x-vercel-ip-continent": "EU" }), EDGE_ENV),
    "https://ams-1.play.rend.so"
  );
});

test("playback base URL prefers the shared metal table in production", () => {
...
test("playback base URL uses the default shared metal route without geo headers", () => {
  const request = new Request("https://www.rend.so/api/player/asset");
...
test("playback base URL uses configured edge map in non-production profiles", () => {
