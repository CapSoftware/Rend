...
test("POST no-ops when telemetry ingest is disabled", async () => {
  const env = process.env as Record<string, string | undefined>;
  const previous = env.REND_PLAYER_TELEMETRY_INGEST;
  ...
  try {
    const response = await POST(
      telemetryRequest({
        playback_session_id: "route-session-disabled",
        asset_id: "asset-123",
        phase: "player_load",
        event_time_ms: EVENT_TIME_MS,
      })
    ...
    assert.equal(response.status, 200);
    assert.deepEqual(await responseJson(response), { status: "ok", accepted: 0 });
  } finally {
    if (previous === undefined) {
      delete env.REND_PLAYER_TELEMETRY_INGEST;
    } else {
