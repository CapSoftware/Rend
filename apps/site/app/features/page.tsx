...
const uploadLines: TerminalLine[] = [
  ...
  { text: "warming opener at the edge", kind: "out" },
    ...
    <div className="overflow-x-clip">
      ...
      <main>
        <PageHero
          ...
          lede={
            <p>
              Send Rend a video and it comes back ready to play behind a single URL, already encoded and
              stored at the edge. You write one API call, not a pipeline, and playback starts from the
              edge nearest each viewer.
            </p>
          }
          actions={
            <>
          ...
          <div className="mt-12 grid grid-cols-1 gap-4 sm:mt-14 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((f) => (
              <Card key={f.id} className="reveal flex flex-col p-6 sm:p-7">
                {f.icon}
                <h2 className="mb-2 mt-5 font-head text-[21px] leading-snug">{f.title}</h2>
                <p className="text-[15px] text-muted">{f.body}</p>
                <ul className="mt-4 flex flex-col gap-2 border-t border-line-soft pt-4">
                  {f.detail.map((d) => (
                    <li key={d} className="flex gap-2.5 text-[14px] leading-snug text-ink-soft">
            ...
            <div className="max-w-[520px]">
              ...
              <ul className="mt-7 flex flex-col gap-4">
                {[
                  ...
                  "Rend encodes the renditions players need and warms an opener at the edge.",
                  ...
                  <li key={step} className="flex gap-3.5">
                    ...
                    <span className="pt-1 text-[15px] leading-snug text-muted">{step}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-8">
        ...
        <Faq faqs={page.faqs} lede="Common questions about what Rend does and how it fits your stack." />
